...
def align_beta_orbitals_to_alpha(mol, orbs):
    """
    Aligns the beta orbitals to match the symmetry irreps of the alpha orbitals.
...
#print("Alpha orbitals:", aligned_alpha_orbs)
#print("Beta orbitals:", aligned_beta_orbs)
nmo = aligned_alpha_orbs.shape[1]
eri_aaaa = pyscf.ao2mo.restore(4,pyscf.ao2mo.incore.general(mf._eri, (aligned_alpha_orbs,aligned_alpha_orbs,aligned_alpha_orbs,aligned_alpha_orbs), compact=False),nmo)
eri_bbbb = pyscf.ao2mo.restore(4,pyscf.ao2mo.incore.general(mf._eri, (aligned_beta_orbs,aligned_beta_orbs,aligned_beta_orbs,aligned_beta_orbs), compact=False),nmo)
eri_aabb = pyscf.ao2mo.restore(4,pyscf.ao2mo.incore.general(mf._eri, (aligned_alpha_orbs,aligned_alpha_orbs,aligned_beta_orbs,aligned_beta_orbs), compact=False),nmo)
h_core = mf.get_hcore(mol)
h_aa = reduce(numpy.dot, (aligned_alpha_orbs.T, h_core, aligned_alpha_orbs))
h_bb = reduce(numpy.dot, (aligned_beta_orbs.T, h_core, aligned_beta_orbs))
nuc = mol.energy_nuc()
float_format = '% 0.20E'
if mol.symmetry:
...
orbsym = pyscf.symm.label_orb_symm(mol,mol.irrep_name,mol.symm_orb,aligned_alpha_orbs)
orbsym = numpy.array(orbsym)
orbsym = [param.IRREP_ID_TABLE[groupname][i]+1 for i in orbsym]
a_inds = [i+1 for i in range(aligned_alpha_orbs.shape[1])]
b_inds = [i+1 for i in range(aligned_beta_orbs.shape[1])]
nelec = mol.nelec
tol=1e-18
